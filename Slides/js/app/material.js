define( ["three", "texture"], function ( THREE, texture ) {
  ...
  return {
    tennisFond :  new THREE.MeshLambertMaterial({
        map: texture.tennisFond,
        shading: THREE.FlatShading
    }),
    tennisBall :  new THREE.MeshLambertMaterial({
            map: texture.tennisBall,
            shininess: 1,
    }),
      cubeTextureCrate : new THREE.MeshPhongMaterial( {map: texture.cubeTextureCrate} ),

      cubeTextureBricks : new THREE.MeshPhongMaterial({ map: texture.cubeTextureBricks}),

      cubeTexture : new THREE.MeshPhongMaterial({ color   : '#ffffff', envMap :  texture.cubeTexture})
